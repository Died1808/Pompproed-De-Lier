...
##############################################K-waarde        
    ...
    def df_TTIM(self, filename):
        ...
        ml=Model3D(kaq=df['k'], z=dfn[ 'depth'], Saq= Ss, kzoverkh=df['kzkh'], tmin=1/1440, 
                    tmax=1, M=10)
        #pompput
        tf = -18  # top Filter
        bf = -35 # onderzijde filter
        ...
        Opgemerkt wordt dat de proef al na 6/8 uur werd beeindigd. Waarschijnlijk 
        was er nog geen evenwicht bereikt, veel data zijn grof. 
      
        """
        laag = np.arange(120,126,5) 
        kleuren(len(laag))
        
        for la in laag:
            ml.xsection(x1=0, x2=1000, y1=0, y2=0, npoints=1000, t=8/24, layers=[la], lw=3, newfig = False)
        ...
        title = (filename +' PP_DeLier'+ '   KD = '+ str(int(KD))+ ' [m2/dag]')
        plt.title(title, fontsize=12, pad=10)
        plt.plot(1,-4.42, 'ko', markersize=5) 
        plt.plot(15,-1.56,'k+', markersize = 20)
        plt.plot(25,-1.39,'k+', markersize = 20)
        plt.plot(45,-1.25,'k+', markersize = 20)
        plt.plot(75,-1.04,'k+', markersize = 20)
        plt.plot(250,-0.51,'k+', markersize = 20)
